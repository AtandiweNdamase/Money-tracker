...
class IncomeExpenseTracker:
    ...
    def view_balance(self):
        ...
        '''with open(filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['TotalIncome', totalIncome])
            writer.writerow(['TotalExpense', totalExpense])
...
def balance():
    mydict = {'12/03/2024': {'Income': [('consultation', 15000.0), ('stipend', 12000.0)], 'Expense': [('car wash', 150.0)]}}
    print(mydict)
    while True:
        ...
        if checkDate =='exit':
            ...
            savebalancecsv(totalIncome, totalExpense,net_balance)
            
def savebalancecsv(totalIncome, totalExpense, net_balance):
    filename = input("Enter filename to save monthly balnce: eg(monthlyAnalysis.csv)")
    with open(filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['TotalIncome', totalIncome])
            writer.writerow(['TotalExpense', totalExpense])
            writer.writerow(['Balance', net_balance])
